<template>
  <div>
    ...
    <p class="tip">
      实现高性能的虚拟树列表：1.拍平树结构 2.构建列表树结构 3.处理展开收缩
    </p>
    ...
    <p>
      <vxe-list height="240" class="my-tree" :loading="loading6" :data="list6">
        <template v-slot="{ items }">
          ...
          >
            <i
              class="tree-icon fa fa-chevron-right"
  ...
  methods: {
    ...
    loadTree6(size) {
      ...
      setTimeout(() => {
        ...
        XEUtils.eachTree(
          trerData,
          (item, index, items, paths, parent, nodes) => {
            // 层级
        ...
        this.$nextTick(() => {
          this.$XModal.message({
            message: `渲染 ${this.fullList6.length} 行，用时 ${
              Date.now() - startTime
            }毫秒`,
            status: "info",
          });
